'''
Author: 璩瑞, Robert Chu
This is the main program to run the CNN architecture, 
including training( -model-type=''), 
		  evaluation(-test, model-type='', snapshot=''), 
		  prediction(-predict, model-type='', snapshot='')
For detail and more info, please refer to README.
'''
#pytorch package
import torch 
...
#####The arguments could be passed when excuting the main.py script#####
######They have default setting value, use main.py --help to learn the info of them#####
...
parser.add_argument('-save-interval', type=int, default=10000, help='how many steps to wait before saving [default:500]')
...
parser.add_argument('-model-type', type=str, default='type1', help='type1: 3-channels input, type2: 1-channel input, both: type1+type2')
#parser.add_argument('-train', action='store_true', default=False, help='train or test')
args = parser.parse_args()

#####Load the numpy array data for specified usage: train, eval, predict#####
#./main for training
if (args.predict is not True) and (args.test is not True):
	print('\nLoad train data.')
	start = time.time()
	train_numpy_data1 = np.load(args.npdir+'/train_input1.npy', mmap_mode='r')
	train_numpy_data2 = np.load(args.npdir+'/train_input2.npy', mmap_mode='r')
	train_numpy_target = np.load(args.npdir+'/train_label.npy', mmap_mode='r')
	train_dataset = MyDataset(train_numpy_data1, train_numpy_data2, train_numpy_target)
...
if (args.test is True) or ((args.predict is not True and args.test is not True) is True):
	print('\nLoad eval data')
	start = time.time()
	eval_numpy_data1 = np.load(args.npdir+'/eval_input1.npy', mmap_mode='r')
	eval_numpy_data2 = np.load(args.npdir+'/eval_input2.npy', mmap_mode='r')
	eval_numpy_target = np.load(args.npdir+'/eval_label.npy', mmap_mode='r')
	eval_dataset = MyDataset(eval_numpy_data1, eval_numpy_data2, eval_numpy_target)
...
if args.predict is True:
	print('\nLoad test data')
	start = time.time()
	test_numpy_data1 = np.load(args.npdir+'/test_input1.npy', mmap_mode='r')
	test_numpy_data2 = np.load(args.npdir+'/test_input2.npy', mmap_mode='r')
	test_numpy_ids = np.load(args.npdir+'/test_id.npy', mmap_mode='r')
	test_dataset = MyDataset(test_numpy_data1, test_numpy_data2, test_numpy_ids)
...
#print(summary(cnn, ([(3, 16, 16),(1, 32, 100)])))
...
if args.predict is True:
	ids, labels = train.predict(testloader, cnn, args)
...
#to train the model
	...
	try:
		...
		if r == -1:
			print('Early stop/quit training at {} steps with best accuracy: {}.'.format(s, a))
		if r ==1:
